...
// ];
export function PersonalInformationForm({ user, userConfig, updateUserAttributes, }) {
    const { handleSubmit, 
    // register,
    isError, error, isSuccess, isLoading, } = useUpdatePersonalInformation({ user, userConfig, updateUserAttributes });
    const { addNotification } = useNotification();
    useEffect(() => {
        if (isSuccess) {
            addNotification({
                type: NotificationType.SUCCESS,
                title: 'Referral Information updated successfully.',
                message: 'Thank you for keeping your Referral information up to date.',
            });
        }
        if (isError && error) {
