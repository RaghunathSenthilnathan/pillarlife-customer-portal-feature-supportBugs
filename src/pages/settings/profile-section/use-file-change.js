...
export function useFileChange() {
    const [{ fileError, fileContents, fileName, fileSize, fileType }, fileDispatch,] = useReducer(fileChangeReducer, initialFileState);
    const handleFileChange = (event) => {
        ...
        if (fileObj.size > MAX_FILE_SIZE_BYTES) {
            fileDispatch({
                type: 'FILE_CHANGE_FAILURE',
                fileError: `File is too large, file size is ${bytesToMb(fileObj.size).toFixed(2)} MB, maximum allowed size - 1 MB.`,
            });
            return;
        }
