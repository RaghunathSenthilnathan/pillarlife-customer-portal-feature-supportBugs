...
const url = `${process.env.FRONTEND_BASE_URL}${ROUTE_PATHS.EDIT_PRIMARY_BENEFICIARY}`;
const title = "Edit Beneficiary";
const description =
...
const editbeneficiary = () => {
  ...
  const [percentsharing, setPercentSharing] = useState([]);
  ...
  function postdata(item) {
    ...
    if (benefi.locator) {
      PrimaryData.locator = benefi.locator;
    } else {
      PrimaryData.id = benefi.id;
    }
    ...
    allstoredbeneficiary.map((item, index) => {
      if (item.locator === benelocator || item.id === benelocator) {
        allstoredbeneficiary.splice(index, 1, PrimaryData);
  ...
  const title = (
    <Container>
      The primary beneficiary details have been updated successfully.
    </Container>
  );
  ...
  return (
    <>
      ...
      <div className="flex md:flex-row md:space-x-1">
        ...
        <div className=" flex flex-col justify-end mt-4 w-1/6">
...
export default withAuthentication(editbeneficiary);
