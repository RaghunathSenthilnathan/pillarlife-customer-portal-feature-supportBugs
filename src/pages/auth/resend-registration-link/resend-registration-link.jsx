...
const description = 'Enables you to resend the registration activation link to your email address.';
export default withAnonymous(ResendRegistrationLink);
function ResendRegistrationLink() {
    const { handleSubmit, register, formErrors, isError, error, isSuccess, isLoading, } = useResendRegistrationLink();
    const { addNotification } = useNotification();
    useEffect(() => {
        if (isSuccess) {
            addNotification({
                type: NotificationType.SUCCESS,
                title: 'Please click on the link, sent to your email address to activate your account.',
    ...
    return (<>
      ...
      <div className="flex flex-col py-12 mt-12 sm:px-6 lg:px-8">
        ...
        })} error={formErrors.email?.message} icon={<EmailIcon />}/>
                  </div>
