...
export const Notification = ({ id, type, title, message, closeNotification, closeNotificationMs = 4000, }) => {
    const [shouldAnimateExit, setShouldAnimateExit] = useState(false);
    const [expirationBarWidth, setExpirationBarWidth] = useState(0);
    const [expirationIntervalId, setExpirationIntervalId,] = useState(null);
    ...
    return (<div onMouseEnter={handlePauseTimer} onMouseLeave={handleStartTimer} className={`w-full max-w-sm overflow-hidden bg-white rounded-lg shadow-lg mb-5 notification-animate-left ${shouldAnimateExit ? 'notification-animate-right' : ''}`}>
      <div className="p-2">
        <div className="flex items-start">
          ...
          <div className="flex flex-shrink-0 ml-2">
            <button type="button" onClick={handleCloseNotification} className="inline-flex text-gray-400 transition duration-150 ease-in-out focus:outline-none focus:text-gray-500">
              <CloseIcon className="w-5 h-5"/>
            </button>
          </div>
        </div>
      </div>
      <div className={`h-1 ${expirationBarColor}`} style={{ width: `${expirationBarWidth}%` }}/>
    </div>);
};
