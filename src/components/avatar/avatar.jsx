...
export const Avatar = ({ src }) => {
    ...
    */
    let faceBookPictureUrl;
    try {
        faceBookPictureUrl = JSON.parse(src).data.url;
    }
    catch (err) {
        faceBookPictureUrl = '';
    }
    if (faceBookPictureUrl.length > 0) {
        return (<img className="inline-block rounded-full h-11 w-11" src={faceBookPictureUrl} alt="avatar"/>);
    }
    const isGoogleProviderPicture = src.includes('https://');
    if (isGoogleProviderPicture) {
        return (<img className="inline-block rounded-full h-11 w-11" src={src} alt="avatar"/>);
    }
    if (src.length > 0) {
        return (<img className="inline-block rounded-full h-11 w-11" src={`${S3_BUCKET_URL}/${src}`} alt="avatar"/>);
    }
    return (<UserIcon className="inline-block text-gray-500 rounded-full h-11 w-11"/>);
};
